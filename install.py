...
umee_home = subprocess.run(
    ["echo $HOME/.umeed"], capture_output=True, shell=True, text=True).stdout.strip()

parser = argparse.ArgumentParser(
    description="Umee Installer", formatter_class=fmt)
...
if not len(sys.argv) > 1:
    parser.set_defaults(mainnetDefault=False, testnetDefault=False, swapOn=None, installHome=None, nodeName=None, ports=None, nodeType=None, network=None, pruning=None, cosmovisorService=None,
                        dataSyncTestnet=None, snapshotTypeTestnet=None, dataSync=None, snapshotType=None, snapshotLocation=None, replayDbBackend=None, extraSwap=None, startReplay=None, releaseFile=None)
...
def partComplete():
    print(bcolors.OKCYAN +
          "Congratulations! You have successfully completed setting up the Umee daemon!")
    print(bcolors.OKCYAN +
...
def replayDelay():
    print(bcolors.OKCYAN +
          "Congratulations! Umee is ready to replay from genesis on your command!")
    print(bcolors.OKCYAN +
...
def localUmeeComplete():
    print(bcolors.OKCYAN +
          "Congratulations! You have successfully completed setting up a LocalUmee node!")
...
def cosmovisorService():
    ...
    subprocess.run(["echo 'export DAEMON_NAME=umeed' >> " +
                   HOME+"/.profile"], shell=True, env=my_env)
    subprocess.run(["echo 'export DAEMON_HOME="+umee_home +
                   "' >> "+HOME+"/.profile"], shell=True, env=my_env)
...
' >umeed.service
    """], shell=True, env=my_env)
    subprocess.run(
        ["sudo mv umeed.service /lib/systemd/system/umeed.service"], shell=True, env=my_env)
    subprocess.run(["sudo systemctl daemon-reload"], shell=True, env=my_env)
    subprocess.run(["systemctl restart systemd-journald"],
                   shell=True, env=my_env)
...
3) Don't install cosmovisor and don't set up a background service
    ...
    if useCosmovisor == "1":
        ...
        subprocess.run(["mkdir -p "+umee_home+"/cosmovisor"],
                       shell=True, env=my_env)
        subprocess.run(
            ["mkdir -p "+umee_home+"/cosmovisor/genesis"], shell=True, env=my_env)
        subprocess.run(
            ["mkdir -p "+umee_home+"/cosmovisor/genesis/bin"], shell=True, env=my_env)
        subprocess.run(
            ["mkdir -p "+umee_home+"/cosmovisor/upgrades"], shell=True, env=my_env)
        subprocess.run(
            ["mkdir -p "+umee_home+"/cosmovisor/upgrades/v7/bin"], shell=True, env=my_env)
        ...
        subprocess.run(["make build"], stdout=subprocess.DEVNULL,
                       stderr=subprocess.DEVNULL, shell=True, env=my_env)
        subprocess.run(["cp build/umeed "+umee_home+"/cosmovisor/upgrades/v7/bin"],
                       stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
        subprocess.run([". "+HOME+"/.profile"], shell=True, env=my_env)
        subprocess.run(["cp " + GOPATH + "/bin/umeed "+umee_home +
...
def replayFromGenesisLevelDb():
    ...
    subprocess.run(["mkdir -p "+umee_home+"/cosmovisor"],
                   shell=True, env=my_env)
    subprocess.run(["mkdir -p "+umee_home+"/cosmovisor/genesis"],
                   shell=True, env=my_env)
    subprocess.run(
        ["mkdir -p "+umee_home+"/cosmovisor/genesis/bin"], shell=True, env=my_env)
    subprocess.run(["mkdir -p "+umee_home+"/cosmovisor/upgrades"],
                   shell=True, env=my_env)
    subprocess.run(
        ["mkdir -p "+umee_home+"/cosmovisor/upgrades/v4/bin"], shell=True, env=my_env)
    subprocess.run(
        ["mkdir -p "+umee_home+"/cosmovisor/upgrades/v5/bin"], shell=True, env=my_env)
    subprocess.run(
        ["mkdir -p "+umee_home+"/cosmovisor/upgrades/v7/bin"], shell=True, env=my_env)
    os.chdir(os.path.expanduser(HOME+"/umee"))
    ...
    subprocess.run(["make build"], stdout=subprocess.DEVNULL,
                   stderr=subprocess.DEVNULL, shell=True, env=my_env)
    subprocess.run(["cp build/umeed "+umee_home+"/cosmovisor/upgrades/v4/bin"],
                   stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
    ...
    subprocess.run(["make build"], stdout=subprocess.DEVNULL,
                   stderr=subprocess.DEVNULL, shell=True, env=my_env)
    subprocess.run(["cp build/umeed "+umee_home+"/cosmovisor/upgrades/v5/bin"],
                   stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
    ...
    subprocess.run(["make build"], stdout=subprocess.DEVNULL,
                   stderr=subprocess.DEVNULL, shell=True, env=my_env)
    subprocess.run(["cp build/umeed "+umee_home+"/cosmovisor/upgrades/v9/bin"],
                   stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
    ...
    subprocess.run(["sed -i -E 's/persistent_peers = \"\"/persistent_peers = \"" +
                   peers+"\"/g' "+umee_home+"/config/config.toml"], shell=True)
    subprocess.run(["clear"], shell=True)
    startReplayNow()
...
def replayFromGenesisRocksDb():
    ...
    subprocess.run(["sed -i -E 's/db_backend = \"goleveldb\"/db_backend = \"rocksdb\"/g' "+umee_home +
    ...
    subprocess.run(["BUILD_TAGS=rocksdb make build"], stdout=subprocess.DEVNULL,
                   stderr=subprocess.DEVNULL, shell=True, env=my_env)
    subprocess.run([". "+HOME+"/.profile"], stdout=subprocess.DEVNULL,
                   stderr=subprocess.DEVNULL, shell=True, env=my_env)
    subprocess.run(["cp build/umeed "+umee_home+"/cosmovisor/genesis/bin"],
                   stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
...
def extraSwap():
    ...
    print(bcolors.OKCYAN + "RAM Detected: " +
...
2) No, do not set up extra swap
        ...
        if swapAns == "1":
            print(bcolors.OKCYAN + "Setting up " +
            ...
            subprocess.run(["echo '/swapfile none swap sw 0 0' | sudo tee -a /etc/fstab"],
                           stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)
            subprocess.run(["clear"], shell=True)
            print(bcolors.OKCYAN + str(swapNeeded) +
...
def infraSnapshotInstall():
    ...
    proc = subprocess.run(["curl https://umee-snapshot.sfo3.cdn.digitaloceanspaces.com/umee.json|jq -r '.[] |select(.file==\"umee-1-pruned\")|.url'"],
...
def snapshotInstall():
    print(bcolors.OKCYAN + "Downloading Decompression Packages..." + bcolors.ENDC)
    if os_name == "Linux":
        install_tools(["wget", "liblz4-tool", "aria2"])
    else:
        install_tools(["aria2", "lz4"])
    print(bcolors.OKCYAN + "Downloading Snapshot..." + bcolors.ENDC)
    proc = subprocess.run(["curl -L https://quicksync.io/umee.json|jq -r '.[] |select(.file==\"" + fileName +
...
3) SanFrancisco (WARNING: Location usually slow)
    ...
    if args.snapshotLocation == "netherlands":
        ...
        nodeLocationAns = input(
            bcolors.OKCYAN + 'Enter Choice: ' + bcolors.ENDC)
...
3) Everything: (modified prune everything due to bug, keep last 10,000 states and prune at a random prime block interval)
    ...
    if pruneAns == "1" and networkAns == "1":
        ...
        subprocess.run(["sed -i -E 's/pruning = \"default\"/pruning = \"nothing\"/g' " +
                       umee_home+"/config/app.toml"], shell=True)
        dataSyncSelection()
        ...
        subprocess.run(["sed -i -E 's/pruning-interval = \"0\"/pruning-interval = \"" +
                       str(primeNum)+"\"/g' "+umee_home+"/config/app.toml"], shell=True)
        dataSyncSelection()
...
2) No, specify custom ports
    ...
    if args.ports:
        ...
        if portChoice == "1":
            ...
            subprocess.run(["clear"], shell=True)
            print(bcolors.OKCYAN +
                  "Input desired values. Press enter for default values" + bcolors.ENDC)
            # app.toml
            ...
            # user input
            api_server = rlinput(
                bcolors.OKCYAN + "(1/7) API Server: " + bcolors.ENDC, api_server_def)
            grpc_server = rlinput(
                bcolors.OKCYAN + "(2/7) gRPC Server: " + bcolors.ENDC, grpc_server_def)
            grpc_web = rlinput(
                bcolors.OKCYAN + "(3/7) gRPC Web: " + bcolors.ENDC, grpc_web_def)
            abci_app_addr = rlinput(
                bcolors.OKCYAN + "(4/7) ABCI Application Address: " + bcolors.ENDC, abci_app_addr_def)
            rpc_laddr = rlinput(
                bcolors.OKCYAN + "(5/7) RPC Listening Address: " + bcolors.ENDC, rpc_laddr_def)
            p2p_laddr = rlinput(
                bcolors.OKCYAN + "(6/7) P2P Listening Address: " + bcolors.ENDC, p2p_laddr_def)
            pprof_laddr = rlinput(
                bcolors.OKCYAN + "(7/7) pprof Listening Address: " + bcolors.ENDC, pprof_laddr_def)
        elif portChoice and portChoice != "1" or portChoice != "2":
            subprocess.run(["clear"], shell=True)
            customPortSelection()
    # change app.toml values
    subprocess.run(["sed -i -E 's|tcp://0.0.0.0:1317|"+api_server +
                   "|g' "+umee_home+"/config/app.toml"], shell=True)
    ...
    subprocess.run(["sed -i -E 's|tcp://127.0.0.1:26658|"+abci_app_addr +
                   "|g' "+umee_home+"/config/config.toml"], shell=True)
...
def setupLocalnet():
    ...
    os.chdir(os.path.expanduser(HOME))
    subprocess.run([f"git clone --depth 1 --branch --single-branch {UMEED_VERSION} https://github.com/umee-network/umee.git.git"],
                   stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)
    os.chdir(os.path.expanduser(HOME+"/umee"))
    subprocess.run(["clear"], shell=True)
    localUmeeComplete()
...
def setupMainnet():
    ...
    subprocess.run(["rm "+umee_home+"/config/app.toml"], stdout=subprocess.DEVNULL,
                   stderr=subprocess.DEVNULL, shell=True, env=my_env)
    subprocess.run(["rm "+umee_home+"/config/config.toml"],
                   stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
    subprocess.run(["rm "+umee_home+"/config/addrbook.json"],
                   stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
    subprocess.run(["umeed init " + nodeName + " --chain-id=umee-1 -o --home "+umee_home],
                   stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
    print(bcolors.OKCYAN + "Downloading and Replacing Genesis..." + bcolors.ENDC)
    subprocess.run(["wget -O "+umee_home+"/config/genesis.json https://raw.githubusercontent.com/umee-network/umee/main/networks/umee-1/genesis.json"],
                   stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
    print(bcolors.OKCYAN + "Downloading and Replacing Addressbook..." + bcolors.ENDC)
    subprocess.run(["wget -O "+umee_home+"/config/addrbook.json https://quicksync.io/addrbook.umee.json"],
                   stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
    subprocess.run(["clear"], shell=True)
    customPortSelection()
...
def setupTestnet():
    ...
    subprocess.run(["rm "+umee_home+"/config/addrbook.json"],
                   stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
    subprocess.run(["umeed init " + nodeName + " --chain-id=umee-test-4 -o --home "+umee_home],
                   stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
    print(bcolors.OKCYAN + "Downloading and Replacing Genesis..." + bcolors.ENDC)
    subprocess.run(["wget -O "+umee_home+"/config/genesis.tar.bz2 wget https://raw.githubusercontent.com/umee-network/umee/main/networks/umeemania-1/genesis.json"],
                   stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
    ...
    subprocess.run(["sed -i -E 's/persistent_peers = \"\"/persistent_peers = \"" +
                   peers+"\"/g' "+umee_home+"/config/config.toml"], shell=True)
    subprocess.run(["tar -xjf "+umee_home+"/config/genesis.tar.bz2"],
                   stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)
    ...
    print(bcolors.OKCYAN + "Downloading and Replacing Addressbook..." + bcolors.ENDC)
    subprocess.run(["wget -O "+umee_home+"/config/addrbook.json https://quicksync.io/addrbook.umeetestnet.json"],
                   stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
    subprocess.run(["clear"], shell=True)
    customPortSelection()
...
def clientSettings():
    if networkAns == "1":
        print(bcolors.OKCYAN + "Initializing Umee Client Node " +
              nodeName + bcolors.ENDC)
        #subprocess.run(["umeed unsafe-reset-all"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
        subprocess.run(["rm "+umee_home+"/config/client.toml"],
                       stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
        subprocess.run(["umeed init " + nodeName + " --chain-id=umee-1 -o --home "+umee_home],
                       stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
        print(bcolors.OKCYAN + "Changing Client Settings..." + bcolors.ENDC)
        subprocess.run(["sed -i -E 's/chain-id = \"\"/chain-id = \"umee-1\"/g' " +
                       umee_home+"/config/client.toml"], shell=True)
        ...
        subprocess.run(["sed -i -E 's|node = \"tcp://localhost:26657\"|node = \"http://umee.artifact-staking.io:26657\"|g' " +
                       umee_home+"/config/client.toml"], shell=True)
        subprocess.run(["clear"], shell=True)
        clientComplete()
    elif networkAns == "2":
        ...
        subprocess.run(["umeed init " + nodeName + " --chain-id=umee-test-4 -o --home "+umee_home],
                       stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
        print(bcolors.OKCYAN + "Changing Client Settings..." + bcolors.ENDC)
        subprocess.run(["sed -i -E 's/chain-id = \"\"/chain-id = \"umee-test-4\"/g' " +
                       umee_home+"/config/client.toml"], shell=True)
        subprocess.run(["sed -i -E 's|node = \"tcp://localhost:26657\"|node = \"https://testnet-rpc.umee.zone:443\"|g' " +
                       umee_home+"/config/client.toml"], shell=True)
        subprocess.run(["clear"], shell=True)
        clientComplete()
    elif networkAns == "3":
        print(bcolors.OKCYAN + "Initializing LocalUmee Node " +
              nodeName + bcolors.ENDC)
        subprocess.run(["rm "+umee_home+"/config/client.toml"],
                       stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
        subprocess.run(["umeed init " + nodeName + " --chain-id=localumee -o --home "+umee_home],
                       stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
        print(bcolors.OKCYAN + "Changing Client Settings..." + bcolors.ENDC)
        subprocess.run(["sed -i -E 's/chain-id = \"\"/chain-id = \"localumee\"/g' " +
                       umee_home+"/config/client.toml"], shell=True)
...
def initNodeName():
    global nodeName
    print(bcolors.OKCYAN +
          "AFTER INPUTTING NODE NAME, ALL PREVIOUS UMEE DATA WILL BE RESET" + bcolors.ENDC)
    if args.nodeName:
        nodeName = args.nodeName
    else:
        nodeName = input(
            bcolors.OKCYAN + "Input desired node name (no quotes, cant be blank): " + bcolors.ENDC)

    if nodeName and networkAns == "1" and node == "1":
        subprocess.run(["clear"], shell=True)
        subprocess.run(["rm -r "+umee_home], stdout=subprocess.DEVNULL,
                       stderr=subprocess.DEVNULL, shell=True, env=my_env)
        subprocess.run(["rm -r "+HOME+"/.umeed"], stdout=subprocess.DEVNULL,
                       stderr=subprocess.DEVNULL, shell=True, env=my_env)
        setupMainnet()
...
def installLocationHandler():
    global umee_home
    print(bcolors.OKCYAN +
          "Input desired installation location. Press enter for default location" + bcolors.ENDC)
    ...
    if args.installHome:
        ...
        umee_home = rlinput(
            bcolors.OKCYAN + "Installation Location: " + bcolors.ENDC, location_def)
    ...
    if umee_home.endswith("/"):
        ...
        umee_home = subprocess.run(
            ["echo "+umee_home], capture_output=True, shell=True, text=True).stdout.strip()
        subprocess.run(["clear"], shell=True)
        initNodeName()
...
def install_tools(names):
    ...
    for i in range(0, len(names)):
        ...
        if not tool_exists(tool_name):
            print(bcolors.OKCYAN + f"Installing {tool_name}..." + bcolors.ENDC)
            if os_name == "Linux":
                subprocess.run([f"sudo apt-get install {tool_name} -y"],
                               stdout=subprocess.DEVNULL, shell=True)
            elif os_name == "Darwin":
                subprocess.run(
                    [f"brew install {tool_name}"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)


def install_go():
    # install go if not exists
    if not tool_exists("go"):
        print(bcolors.OKCYAN + "Installing Go..." + bcolors.ENDC)
        subprocess.run(["wget -q -O - https://git.io/vQhTU | bash -s -- --version 1.19"],
                       stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)

    print(bcolors.OKCYAN + "Reloading Profile..." + bcolors.ENDC)
    subprocess.run(["source ~/.profile"], stdout=subprocess.DEVNULL,
                   stderr=subprocess.DEVNULL, shell=True)
...
def install_docker():
    ...
    if not tool_exists("docker"):
        subprocess.run(["wget -q -O - https://get.docker.com | bash"],
                       stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)
    install_tools(["docker-compose"])
...
def initSetup():
    ...
    if os_name == "Linux":
        print(bcolors.OKCYAN +
              "Please wait while the following processes run:" + bcolors.ENDC)
        ...
        print(bcolors.OKCYAN +
              f"Installing Umee {UMEED_VERSION} Binary..." + bcolors.ENDC)
        subprocess.run(["make install"], stdout=subprocess.DEVNULL,
                       stderr=subprocess.DEVNULL, shell=True, env=my_env)
        if node == "3":
            install_docker()
        ...
        print(bcolors.OKCYAN +
              "Please wait while the following processes run:" + bcolors.ENDC)
        print(bcolors.OKCYAN +
              "(1/4) Checking for brew and wget. If not present, installing..." + bcolors.ENDC)
        ...
        my_env = os.environ.copy()
        print(bcolors.OKCYAN +
              "f(4/4) Installing Umee {UMEED_VERSION} Binary..." + bcolors.ENDC)
...
def read_latest_version(file=None, loc=None):
    global latest_releases
    if loc:
        with open(file, "r") as f:
            latest_releases = json.load(f)
    else:
        # Read from uri
        response = urlopen(url=file)
        latest_releases = json.loads(response.read())
...
Welcome to the Umee node installer """+UMEED_VERSION+"""
...
3) LocalUmee Node (setup a daemon and query a localUmee development RPC)
        ...
        # print("args.nodeType",args.nodeType)
